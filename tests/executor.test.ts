...
describe('Resolver Engine', () => {
  it('should magically resolve relation IDs via lookup string', async () => {
    ...
    const payload = {
      ...
      find: vi.fn().mockResolvedValue({ docs: [{ id: 'mock-resolved-id-123' }] })
    };
    ...
    const result = await resolveRelations(payload as any, 'posts', data, config);
...
describe('Executor Engine', () => {
  it('should perform single create action with basic data', async () => {
    ...
    await execute(payload as any, 'users', 'create', JSON.stringify({ name: 'Test' }), config);
