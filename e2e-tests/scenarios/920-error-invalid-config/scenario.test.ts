...
describe('Error invalid config', () => {
  ...
  it('fails when an invalid onNotFound value is provided via -j', () => {
    ...
    const output = runCLI(`-j '{"mappings":{"users":{"onNotFound":"invalid-action"}}}' users upsert ${dataPath}`);
  ...
  it('fails when lookupField is not a string', () => {
    ...
    const output = runCLI(`-j '{"mappings":{"users":{"lookupField": 123}}}' users upsert ${dataPath}`);
